...
function initRouter(debug, ROOT_PATH) {
	...
	console.log("[AXLE-LOG]路由配置keys:" + Object.keys(apiConf));
	...
	async.forEach(Object.keys(apiConf), function (appName, done){ 
		...
		pages.forEach(function(pageItem) {
			...
			console.log('[AXLE-LOG]页面路由:' + routeUrl)
			...
			router.get(routeUrl, function(req, res, next) {
				...
				console.log('[AXLE-LOG]页面view路径:' + pagePath)
...
module.exports = function(debug, ROOT_PATH) {
	//huya.com域登录时的回调，必须要有这个回调页面去处理huya登录的请求
	router.get('/udbcallback.html', function(req, res) {
		res.sendFile(ROOT_PATH + '/views/udbcallback.html');
	});
